...
export const FEATURE_REGISTRY: Record<string, FeatureDescription> = {
  ...
  customer_credit_panel: {
    ...
    shortDescription: "Per-customer Accounting & Credit visibility panel — open AR, past due, payment terms, credit limit, manual + policy hold state. Booking-flow enforcement reads from this state.",
    guideDescription: "The Accounting & Credit panel on the customer detail page shows the full credit posture for a customer in one place: total open AR, total past due, oldest past-due age, the effective payment terms (with source — customer override vs tenant default vs system default), the effective credit limit and available credit, and the hold state (manual + policy + effective). When a hold is active the panel surfaces structured reason cards explaining why. Admin and owner roles see inline controls to edit payment terms, set credit limit, and set or release a manual hold with a required reason. ENFORCEMENT (Phase 4): the booking flow now respects this state. When you start a new booking for a customer who is on hold, the BookingWizard surfaces a credit warning or block banner depending on the tenant policy mode (warn vs block). Manual holds always block. Block-mode policy holds also block. Warn-mode policy holds show a warning but allow the booking to proceed. When 'Allow Office Override' is enabled in the tenant credit policy, admin and owner roles see an 'Override & Continue' button on blocked bookings — applying an override requires a non-empty reason and records the override on the new job's notes for audit (format: '[Credit Override] {reason} (by {userId} at {timestamp})'). Dispatch, the Jobs page, and existing scheduled jobs are NOT affected — only new booking flows. The hold state is re-evaluated live from the API on every customer selection.",
    routeOrSurface: "customer_detail", tenantOverrideKey: "customer_credit_panel",
    isUserFacing: true, isGuideEligible: true,
    keywords: ["accounting", "credit", "ar", "past due", "payment terms", "credit limit", "hold", "billing", "invoice", "customer", "booking", "enforcement", "override"],
  },
