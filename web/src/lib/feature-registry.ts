...
export const FEATURE_REGISTRY: Record<string, FeatureDescription> = {
  ...
  customer_credit_panel: {
    ...
    shortDescription: "Per-customer Accounting & Credit visibility panel — open AR, past due, payment terms, credit limit, manual + policy hold state.",
    guideDescription: "The Accounting & Credit panel on the customer detail page shows the full credit posture for a customer in one place: total open AR, total past due, oldest past-due age, the effective payment terms (with source — customer override vs tenant default vs system default), the effective credit limit and available credit, and the hold state (manual + policy + effective). When a hold is active the panel surfaces structured reason cards explaining why. Admin and owner roles see inline controls to edit payment terms, set credit limit, and set or release a manual hold with a required reason. IMPORTANT: enforcement is not yet active. Setting a hold or exceeding a credit limit is currently visibility-only — the dispatch board, booking flow, and blocked-job drawer do not yet read from this state. Future phases will wire enforcement on top of this panel. Use this phase to validate that the data looks right for your tenant before enforcement turns on.",
    routeOrSurface: "customer_detail", tenantOverrideKey: "customer_credit_panel",
    isUserFacing: true, isGuideEligible: true,
    keywords: ["accounting", "credit", "ar", "past due", "payment terms", "credit limit", "hold", "billing", "invoice", "customer"],
  },
  ...
  tenant_credit_policy_visibility_only_notice: {
    id: "tenant_credit_policy_visibility_only_notice", label: "Visibility only — enforcement not active yet.", category: "settings",
    shortDescription: "Disclaimer shown on the credit policy section to make clear that policy values are stored but not enforced in this phase.",
    guideDescription: "",
    routeOrSurface: "settings", tenantOverrideKey: "tenant_credit_policy_visibility_only_notice",
    isUserFacing: true, isGuideEligible: false,
    keywords: ["visibility only", "not enforced"],
  },
};
