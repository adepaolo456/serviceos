...
export default function PricingPage() {
  ...
  return (
    <div>
      ...
      {/* Price Calculator */}
      <div className="mb-8 rounded-[14px] border border-[var(--t-border)] bg-[var(--t-bg-card)] overflow-hidden">
        <button
      ...
      {loading ? (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-48 skeleton rounded-[14px]" />
          ))}
        </div>
      ) : rules.length === 0 ? (
...
function RuleCard({ rule, onEdit }: { rule: PricingRule; onEdit: () => void }) {
  return (
    <div
      onClick={onEdit}
      className={`group relative cursor-pointer rounded-[14px] border border-[var(--t-border)] bg-[var(--t-bg-card)] p-5 transition-colors hover:bg-[var(--t-bg-card-hover)] ${
