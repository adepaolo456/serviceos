...
export default function CustomerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      {/* Back */}
      <Link href="/customers" className="inline-flex items-center gap-1.5 text-xs text-[var(--t-text-muted)] hover:text-[var(--t-text-primary)] transition-colors mb-4">
      ...
      {/* ===== TABS ===== */}
      <div className="flex gap-0 border-b border-[var(--t-border)] mb-5 overflow-x-auto">
        {TABS.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`relative flex items-center gap-1.5 px-4 py-2.5 text-xs font-medium transition-colors shrink-0 ${tab === t.key ? "text-[var(--t-accent)]" : "text-[var(--t-text-muted)] hover:text-[var(--t-text-primary)]"}`}>
            <t.icon className="h-3.5 w-3.5" />
            {t.label}
            {t.key === "jobs" && <span className="text-[9px] text-[var(--t-text-muted)] ml-0.5">{jobs.length}</span>}
            {t.key === "invoices" && <span className="text-[9px] text-[var(--t-text-muted)] ml-0.5">{invoices.length}</span>}
            {t.key === "notes" && notes.length > 0 && <span className="text-[9px] text-[var(--t-text-muted)] ml-0.5">{notes.length}</span>}
