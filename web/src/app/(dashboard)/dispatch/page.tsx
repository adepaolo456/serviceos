  ...
  if (collapsed) {
    return (
      <div
        ...
        onDrop={(e) => { onDrop(e); setCollapsed(false); }}
        className={`flex w-[60px] shrink-0 cursor-pointer flex-col items-center rounded-xl bg-[#111C2E] border py-3 hover:bg-[#162033] transition-all ${
          isDropTarget ? "border-[#2ECC71] border-dashed ring-2 ring-[#2ECC71]/30 bg-[#2ECC71]/5" : "border-[#1E2D45]"
        }`}
      >
...
function JobCard({ job, order, drivers, onAssign, onQuickView, onDragStart, onDragEnd }: {
  ...
  return (
    ...
    >
      ...
      {/* Card body — click opens QuickView */}
      <button onClick={() => onQuickView?.(job)} className="block w-full text-left">
        <div className={`rounded-lg bg-[#162033] border border-[#1E2D45] border-l-[3px] ${statusBorder} p-2.5 pl-7 transition-all hover:border-[#2ECC71]/20 cursor-grab active:cursor-grabbing ${isCompleted ? "opacity-60" : ""}`}>
          {/* Grip handle */}
          <div className="absolute left-1.5 top-1/2 -translate-y-1/2 text-muted/20 group-hover:text-muted/50">
          ...
          <div className="flex items-start justify-between gap-2 mb-1.5">
            ...
            <div className="flex items-center gap-1 shrink-0">
              {job.asset?.identifier && (
                <span className="rounded bg-brand/10 text-brand px-1.5 py-0.5 text-[9px] font-bold">{job.asset.identifier}</span>
              )}
