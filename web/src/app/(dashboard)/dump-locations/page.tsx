...
const inputCls =
  "w-full rounded-[14px] border border-[var(--t-border)] bg-[var(--t-bg-card)] px-3 py-2 text-sm text-[var(--t-text-primary)] placeholder-[var(--t-text-muted)] outline-none focus:border-[var(--t-accent)]";
...
export default function DumpLocationsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Add form */}
      {adding && (
        <div className="rounded-[14px] border border-[var(--t-accent)] bg-[var(--t-bg-card)] p-5 space-y-3">
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-24 skeleton rounded-[14px]" />
          ))}
        </div>
      ) : locations.length === 0 ? (
