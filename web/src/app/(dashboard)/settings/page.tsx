...
export default function SettingsPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-[28px] font-bold tracking-[-1px] text-[var(--t-text-primary)]">Settings</h1>
        <p className="mt-1 text-[13px] text-[var(--t-text-muted)]">Manage your account and preferences</p>
      </div>
          ...
          return (
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium transition-colors shrink-0 ${
                tab === t.key ? "bg-[var(--t-accent-soft)] text-[var(--t-accent)]" : "text-[var(--t-text-muted)] hover:text-[var(--t-text-primary)]"
              }`}>
