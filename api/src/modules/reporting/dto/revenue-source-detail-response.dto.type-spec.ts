/**
 * Type-only sanity check for `RevenueSourceDetailResponseDto`.
 *
 ...
 *      TS2578).
 *
 * Phase 4a note: this is the FIRST consumer of the shared
 * `RevenueDetailInvoiceDto` (extracted in Phase 4-extract `ce0b7e7`).
 * The positive fixture uses runtime-accurate types — `createdAt` as
 * `Date` (pg driver returns TIMESTAMP as Date), `invoiceNumber` as
 * `number` (INT4), `jobId`/`jobNumber` nullable (LEFT JOIN may miss).
 * If this fixture fails to compile, it indicates an extraction defect
 * in `revenue-detail-invoice.dto.ts` — NOT a fixture bug.
 *
 * The `.type-spec.ts` suffix matches Phase 1 v2 / Phase 2 / Phase 3
 * template: Jest globs `.spec.ts` only, so this file is picked up by
 * `tsc` but NOT by `jest`.
 */

import type { RevenueSourceDetailResponseDto } from './revenue-source-detail-response.dto';
...
const _validFixture: RevenueSourceDetailResponseDto = {
  source: 'google_ads',
  invoices: [
    {
      id: 'inv-uuid-1',
...
//      `topCustomers[].totalSpend` sentinel choice.
// ─────────────────────────────────────────────────────────────────────

const _invalidFixture: RevenueSourceDetailResponseDto = {
  ..._validFixture,
  invoices: [
    {
