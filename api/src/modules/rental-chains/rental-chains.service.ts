...
export class RentalChainsService {
  ...
  async createChain(tenantId: string, dto: CreateRentalChainDto) {
    ...
    // Create drop-off job
    const dateStr = dto.drop_off_date.replace(/-/g, '');
    const rand = Math.random().toString(36).slice(2, 5).toUpperCase();

    const dropOffJob = this.jobRepo.create({
      tenant_id: tenantId,
      customer_id: dto.customer_id,
      job_number: `JOB-${dateStr}-${rand}D`,
      job_type: 'delivery',
  ...
  async handleTypeChange(tenantId: string, jobId: string, oldType: string, newType: string) {
    const link = await this.linkRepo.findOne({ where: { job_id: jobId } });
    if (!link) return;

    const chain = await this.chainRepo.findOne({ where: { id: link.rental_chain_id } });
    if (!chain) return;

    if (oldType === 'exchange' && newType === 'pick_up') {
      // Cancel the next link (auto-scheduled pickup for new dumpster)
      if (link.next_link_id) {
        const nextLink = await this.linkRepo.findOne({ where: { id: link.next_link_id } });
        if (nextLink) {
          nextLink.status = 'cancelled';
          await this.linkRepo.save(nextLink);
          // Cancel its job
          await this.jobRepo.update(nextLink.job_id, { status: 'cancelled', cancelled_at: new Date() });
        }
      ...
      // Check if this is the final active task
      const activeLinks = await this.linkRepo.count({
        where: { rental_chain_id: chain.id, status: 'scheduled' },
      });
      if (activeLinks <= 1) {
    ...
    if (newType === 'exchange') {
      // Cancel current pickup link
      const currentPickup = await this.linkRepo.findOne({
        where: { rental_chain_id: chain.id, task_type: 'pick_up', status: 'scheduled' },
      });

      if (currentPickup) {
        currentPickup.status = 'cancelled';
        await this.linkRepo.save(currentPickup);
        await this.jobRepo.update(currentPickup.job_id, { status: 'cancelled', cancelled_at: new Date() });
      }

      // Get max sequence
      const maxSeq = await this.linkRepo
      ...
      // Create exchange link (reuse existing job)
  ...
  async updateLinkStatus(tenantId: string, chainId: string, linkId: string, status: string) {
    const chain = await this.findOne(tenantId, chainId);
    const link = chain.links?.find(l => l.id === linkId);
    if (!link) throw new NotFoundException(`Link ${linkId} not found in chain`);
